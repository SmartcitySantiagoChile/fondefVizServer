...
class LocalInfoViewTest(TestCase):
    ...
    def test_OPDictionaryCsvUploader_post(self):
        with open(os.path.join(self.path, 'opdictionary.csv'), 'r') as file:
            response = self.client.post('/csvuploadOP/', {'name': 'file.csv', 'csvDictionary': file})
        ...
        bad_response = self.client.post('/csvuploadOP/', {'name': 'file.csv', 'csvDictionary': ''})
        self.assertEqual(400, bad_response.status_code)

    def test_CustomRouteCsvUploader_post(self):
        with open(os.path.join(self.path, 'opdictionary.csv'), 'r') as file:
            response = self.client.post('/csvupload/', {'name': 'file.csv', 'csvDictionary': file})
        ...
        bad_response = self.client.post('/csvupload/', {'name': 'file.csv', 'csvDictionary': ''})
        self.assertEqual(400, bad_response.status_code)
