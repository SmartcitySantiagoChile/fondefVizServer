...
POST_PRODUCTS_STAGE_TRANSFERS_AGGREGATED_DATA = (
    "post_products_stage_transfers_aggregated_data"
)
POST_PRODUCTS_TRIP_TRIP_BETWEEN_ZONES_DATA = (
    "post_products_trip_trip_between_zones_data"
)
POST_PRODUCTS_TRIP_BOARDING_AND_ALIGHTING_DATA = (
    "post_products_trip_boarding_and_alighting_data"
)
POST_PRODUCTS_TRIP_BOARDING_AND_ALIGHTING_WITHOUT_SERVICE_DATA = (
    "post_products_trip_boarding_and_alighting_without_service_data "
)
POST_PRODUCTS_STAGE_TRANSACTIONS_BY_OPERATOR_DATA = (
    "post_products_stage_transactions_by_operator_data"
)
...
class ZipManager:
    ...
    def __init__(self, file_path):
        ...
        self.zip_file_obj = zipfile.ZipFile(
            self.file_path, mode="w", compression=zipfile.ZIP_DEFLATED, allowZip64=True
        )
...
class CSVHelper:
    ...
    def __init__(self, es_client, es_query, index_name):
        ...
        self.halfhour_dict = get_halfhour_list_for_select_input(
            to_dict=True, format="name"
        )
        self.commune_dict = get_commune_list_for_select_input(to_dict=True)
        self.transport_mode_dict = get_transport_mode_list_for_select_input(
            to_dict=True
        )
    ...
    def get_field_explanation(self):
        explanation = "El archivo {0} contiene las siguientes columnas:\r\n".format(
            self.get_data_file_name()
        )
        explanation += "\r\n".join(
            [
                "\t- '{0}': {1}".format(field["csv_name"], field["definition"])
                for field in self.get_column_dict()
            ]
        )
        return explanation
    ...
    def _process_filters(self, filters):
        ...
        for query_filter in filters:
            if "term" in query_filter:
                ...
                attr_filter = {"field": self.translator[field], "value": value}
                ...
                compare_values = list(
                    map(
                        lambda x: x.replace("||/d", "") if type(x) is str else x,
                        [gte, lte, gt, lt],
                    )
                )
                if None not in compare_values[0:2]:
                    ...
                    if values_are_dates:
                        attr_filter[
                            "value"
                        ] = "entre {0} 00:00:00 y {1} 23:59:59".format(
                            lower_bound, upper_bound
                        )
                    else:
                        attr_filter["value"] = "entre {0} y {1}".format(
                            lower_bound, upper_bound
                        )
    ...
    def _formatter_for_web(self, elements, sep=" y "):
        ...
        for element in elements:
            if "group" in element:
                format_str = (
                    "{0}({1})" if element["group"] == "should" else "{0} no {1}"
                )
                description += format_str.format(
                    sep, self._formatter_for_web(element["value"], " o ")
                )
            else:
                value = element["value"]
                ...
                description += "{0}{1}: {2}{3}".format(
                    sep, element["field"], value, break_line
                )
    ...
    def _formatter_for_file(
        ...
        for element in elements:
            if "group" in element:
                format_str = (
                    "{0}({1}){2}" if element["group"] == "should" else "{0} no {1}{2}"
                )
                description += format_str.format(
                    sep,
                    self._formatter_for_file(element["value"], " o ", "", True),
                    break_line,
                )
            else:
                value = element["value"]
...
class ProfileCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "expeditionStopOrder",
                "csv_name": "Secuencia_parada",
                "definition": "Posición de la parada dentro de la secuencia de paradas asociada al servicio",
            },
            {
                "es_name": "expeditionDayId",
                "csv_name": "Identificador_expedición_día",
                "definition": "identificador de la expedición, es único dentro del día",
            },
            {
                "es_name": "stopDistanceFromPathStart",
                "csv_name": "Distancia_parada_desde_inicio_ruta",
            ...
            {
                "es_name": "expeditionStopTime",
                "csv_name": "Hora_en_parada",
                "definition": "Fecha y hora en que la máquina pasó por la parada",
            },
            {
                "es_name": "timePeriodInStartTime",
                "csv_name": "Periodo_transantiago_inicio_expedicion",
                "definition": "Período transantiago en que inició la expedición",
            },
            {
                "es_name": "timePeriodInStopTime",
                "csv_name": "Periodo_transantiago_parada_expedición",
            ...
            {
                "es_name": "uniformDistributionMethod",
                "csv_name": "uniforme",
            ...
            {
                "es_name": "passengerPerKmSection",
                "csv_name": "Pax-km_tramo",
                "definition": "Carga X distancia entre parada anterior y la del registro",
            },
            {
                "es_name": "passengerWithEvasionPerKmSection",
                "csv_name": "Pax-km_corregido_tramo",
                "definition": "Carga corregida X distancia entre parada anterior y la del registro",
            },
            {
                "es_name": "capacityPerKmSection",
                "csv_name": "Plazas-km_tramo",
    ...
    def get_file_description(self):
        description = (
            "archivo de perfiles de carga, cada fila representa el cruce de una expedición por una parada "
...
class SpeedCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "authRouteCode",
                "csv_name": "Servicio_transantiago",
                "definition": "Código transantiago del servicio",
            },
            {
                "es_name": "userRouteCode",
                "csv_name": "Servicio_usuario",
                "definition": "Código usuario del servicio",
            },
            {
                "es_name": "section",
                "csv_name": "Identificador_tramo",
            ...
            {
                "es_name": "date",
                "csv_name": "Fecha_de_medición",
                "definition": "Fecha de la medición",
            },
            {
                "es_name": "periodId",
                "csv_name": "Período_de_medición",
                "definition": "Período de media hora de la medición",
            },
            {
                "es_name": "dayType",
                "csv_name": "Tipo_día",
                "definition": "Tipo de día en que se realizó la medicion",
            },
            {
                "es_name": "totalDistance",
                "csv_name": "Distancia_total",
                "definition": "Distancia recorrida por las expediciones que cruzaron en el tramo en el período de medición",
            },
            {
                "es_name": "totalTime",
                "csv_name": "Tiempo_total",
                "definition": "Tiempo usado por las expediciones que cruzaron el tramo en el período de medición",
            },
            {
                "es_name": "speed",
                "csv_name": "Velocidad_m/s",
                "definition": "Velocidad del tramo, período",
            },
            {
                "es_name": "nObs",
                "csv_name": "Número_observaciones",
                "definition": "Número de puntos gps utilizados para realizar el cálculo de la velocidad",
            },
            {
                "es_name": "nInvalidObs",
                "csv_name": "Número_observaciones_inválidas",
    ...
    def get_file_description(self):
        description = (
            "Cada línea representa la medición de la velocidad para un servicio cada 500 metros en "
...
class ODByRouteCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            },
            {
                "es_name": "dayType",
                "csv_name": "Tipo_día",
                "definition": "Tipo de día en que inició la etapa",
            },
            {
                "es_name": "operator",
                "csv_name": "Operador",
            ...
            {
                "es_name": "timePeriodInStopTime",
                "csv_name": "Período_transantiago_subida",
                "definition": "Período transantiago en que la expedición pasó por la parada",
            },
            {
                "es_name": "startStopOrder",
                "csv_name": "Posición_parada_en_ruta_subida",
                "definition": "Posición de la parada de subida en la ruta del servicio",
            },
            {
                "es_name": "endStopOrder",
                "csv_name": "Posición_parada_en_ruta_bajada",
                "definition": "Posición de la parada de bajada en la ruta del servicio",
            },
            {
                "es_name": "authStartStopCode",
                "csv_name": "Código_transantiago_parada_subida",
                "definition": "Código transantiago de la parada de subida",
            },
            {
                "es_name": "authEndStopCode",
                "csv_name": "Código_transantiago_parada_bajada",
                "definition": "Código transantiago de la parada de bajada",
            },
            {
                "es_name": "userStartStopCode",
                "csv_name": "Código_usuario_parada_subida",
                "definition": "Código usuario de la parada de subida",
            },
            {
                "es_name": "userEndStopCode",
                "csv_name": "Código_usuario_parada_bajada",
                "definition": "Código usuario de la parada de bajada",
            },
            {
                "es_name": "startStopName",
                "csv_name": "Nombre_parada_subida",
    ...
    def get_file_description(self):
        description = (
            "Cada línea representa la cantidad de etapas realizadas entre dos paradas en un período "
            "transantiago."
        )
        return "\t\t- {0}: {1}\r\n".format(self.get_data_file_name(), description)
...
class TripCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "factor_expansion",
                "csv_name": "Factor_expansion",
            ...
            {
                "es_name": "tviaje",
                "csv_name": "Tiempo_viaje",
                "definition": "Duración del viaje en segundos",
            },
            {
                "es_name": "distancia_eucl",
                "csv_name": "Distancia_euclidiana",
            ...
            },
            {
                ...
                "csv_name": "Tiempo_bajada",
                "definition": "Fecha y hora en que terminó el viaje",
            },
            {
                "es_name": "mediahora_subida",
                "csv_name": "Media_hora_subida",
                "definition": "Tramo de media hora en que inició el viaje",
            },
            {
                "es_name": "mediahora_bajada",
                "csv_name": "Media_hora_bajada",
                "definition": "Tramo de media hora en que finalizó el viaje",
            },
            {
                "es_name": "periodo_subida",
                "csv_name": "Periodo_transantiago_subida",
                "definition": "Período transantiago en que inició el viaje",
            },
            {
                "es_name": "periodo_bajada",
                "csv_name": "Período_transantiago_bajada",
                "definition": "Período transantiago en que finalizó el viaje",
            },
            {
                "es_name": "tipo_transporte_1",
                "csv_name": "Tipo_transporte_etapa_1",
                "definition": "Modo de transporte utilizado en la etapa 1",
            },
            {
                "es_name": "tipo_transporte_2",
                "csv_name": "Tipo_transporte_etapa_1",
                "definition": "Modo de transporte utilizado en la etapa 2",
            },
            {
                "es_name": "tipo_transporte_3",
                "csv_name": "Tipo_transporte_etapa_1",
                "definition": "Modo de transporte utilizado en la etapa 3",
            },
            {
                "es_name": "tipo_transporte_4",
                "csv_name": "Tipo_transporte_etapa_1",
                "definition": "Modo de transporte utilizado en la etapa 4",
            },
            {
                "es_name": "srv_1",
                "csv_name": "Servicio_etapa_1",
                "definition": "Servicio utilizado en la etapa 1",
            },
            {
                "es_name": "srv_2",
                "csv_name": "Servicio_etapa_2",
                "definition": "Servicio utilizado en la etapa 2",
            },
            {
                "es_name": "srv_3",
                "csv_name": "Servicio_etapa_3",
                "definition": "Servicio utilizado en la etapa 3",
            },
            {
                "es_name": "srv_4",
                "csv_name": "Servicio_etapa_4",
                "definition": "Servicio utilizado en la etapa 4",
            },
            {
                "es_name": "paradero_subida",
                "csv_name": "Parada_subida",
                "definition": "Código transantiago de la parada donde inició el viaje",
            },
            {
                "es_name": "paradero_bajada",
                "csv_name": "Parada_bajada",
                "definition": "Código transantiago de la parada donde finalizó el viaje",
            },
            {
                "es_name": "comuna_subida",
                "csv_name": "Comuna_subida",
                "definition": "Comuna asociada a la parada de subida de la primera etapa del viaje",
            },
            {
                "es_name": "comuna_bajada",
                "csv_name": "Comuna_bajada",
...
class ShapeCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        ...
        return [
            ...
            {
                "es_name": "startDate",
                "csv_name": "Fecha_inicio_programa_operación",
    ...
    def get_iterator(self, kwargs):
        ...
        return [
            self.es_shape_helper.get_route_shape(route, [[start_date, end_date]])
            for route in routes
        ]
...
class StopByRouteCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        ...
        return [
            ...
            {"es_name": "", "csv_name": "Longitud", "definition": "longitud"},
            {
                "es_name": "",
                "csv_name": "Código_parada_transantiago",
                "definition": "Código transantiago de la parada",
            },
            {
                "es_name": "",
                "csv_name": "Código_parada_usuario",
                "definition": "Código usuario de la parada",
            },
            {
                "es_name": "",
                "csv_name": "Nombre_parada",
                "definition": "Nombre de la parada",
            },
        ]
    ...
    def get_iterator(self, kwargs):
        ...
        return [
            self.es_stop_helper.get_stop_list(route, [[start_date, end_date]])
            for route in routes
        ]
...
class PaymentFactorCSVHelper(CSVHelper):
    ...
    def __init__(self, es_client, es_query):
        CSVHelper.__init__(
            self, es_client, es_query, ESPaymentFactorHelper().index_name
        )
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "busStationId",
                "csv_name": "Identificador_zona_paga",
                "definition": "Código que identifica la zona paga",
            },
            {
                "es_name": "busStationName",
                "csv_name": "Nombre_zona_paga",
...
class BipCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "validationTime",
                "csv_name": "Hora_validacion",
                "definition": "Fecha y hora de la validación",
            },
            {
                "es_name": "bipNumber",
                "csv_name": "Número Bip",
                "definition": "Número de la tarjeta bip",
            },
            {
                "es_name": "source",
                "csv_name": "Fuente de dato",
    ...
    def get_file_description(self):
        description = (
            "archivo de transacciones bip, cada fila representa una validación bip"
        )
        return "\t\t- {0}: {1}\r\n".format(self.get_data_file_name(), description)
...
class FormattedShapeCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        ...
        return [
            ...
            {
                "es_name": "",
                "csv_name": "Id_segmento",
                "definition": "Id de segmento 500m",
            },
            {"es_name": "", "csv_name": "Latitud", "definition": "Latitud"},
            {"es_name": "", "csv_name": "Longitud", "definition": "Longitud"},
        ]
    ...
    def row_parser(self, row):
        ...
        if last_segment["segmentStart"] == 1:
            rows.append(
                [route, counter, last_segment["latitude"], last_segment["longitude"]]
            )
...
class PostProductsStageTransferInPeriodCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "halfHourInBoardingTime",
                "csv_name": "Media_hora_subida",
                "definition": "Tramo de media hora en que inició el viaje",
            },
            {
                "es_name": "expandedBoarding",
                "csv_name": "Número_transbordos",
            ...
            {
                "es_name": "boardingTime",
                "csv_name": "Tiempo_subida",
                "definition": "Fecha y hora en que se inició el viaje",
            },
            {
                "es_name": "stageNumber",
                "csv_name": "Número_etapa",
...
class PostProductsStageTransferInPeriodGroupedByDateCSVHelper(CSVHelper):
    ...
    def get_file_description(self):
        description = (
            "Cada línea representa un conjunto de transbordos en una parada por día."
        )
        return "\t\t- {0}: {1}\r\n".format(self.get_data_file_name(), description)
...
class PostProductTripTripBetweenZonesCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "startCommune",
                "csv_name": "Comuna_origen",
                "definition": "Comuna de inicio del viaje",
            },
            {
                "es_name": "endCommune",
                "csv_name": "Comuna_destino",
                "definition": "Comuna de destino del viaje",
            },
            {
                "es_name": "transportModes",
                "csv_name": "Modos_de_transporte",
            ...
            {
                "es_name": "tripNumber",
                "csv_name": "Cantidad_de_viajes",
                "definition": "Suma de viajes expandidos en la agrupación",
            },
            {
                "es_name": "expandedStages",
                "csv_name": "Cantidad_de_etapas",
                "definition": "Número de etapas promedio",
            },
            {
                "es_name": "expandedTime",
                "csv_name": "Tiempo_de_viaje",
                "definition": "Tiempo de viaje promedio",
            },
            {
                "es_name": "expandedDistance",
                "csv_name": "Distancia_de_viaje",
                "definition": "Distancia de viaje promedio",
            },
            {
                "es_name": "speed",
                "csv_name": "Velocidad_de_viaje",
                "definition": "Velocidad de viaje promedio",
            },
            # extra columns, está columna existe para el diccionario que aparece en la sección de descarga
            {
                "es_name": "tiempo_subida",
            ...
            for end_commune in start_commune.endCommune:
                ...
                for transport_modes in end_commune.transportModes:
                    ...
                    for (
                        ...
                        sum_trip_distance = (
                            half_hour_in_boarding_time.expandedDistance.value
                        )
                        sum_trip_stages = (
                            half_hour_in_boarding_time.expandedStages.value
                        )
                        # convert h to m
...
class PostProductTripBoardingAndAlightingCSVHelper(CSVHelper):
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "expandedBoarding",
                "csv_name": "Cantidad_de_subidas",
                "definition": "Suma de viajes expandidos en la agrupación",
            },
            {
                "es_name": "expandedAlighting",
                "csv_name": "Cantidad_de_bajadas",
    ...
    def get_file_description(self):
        description = (
            "Cada línea representa un conjunto de subidas y bajadas por parada."
        )
        return "\t\t- {0}: {1}\r\n".format(self.get_data_file_name(), description)
            ...
            for stop in commune.authStopCode:
                ...
                for transport_modes in stop.transportModes:
                    if transport_modes.key != 0:
                        transport_modes_str = self.transport_mode_dict[
                            transport_modes.key
                        ]
                    else:
                        continue
                    for auth_route in transport_modes.authRouteCode:
                        ...
                        for (
                            ...
                            sum_trip_number = (
                                half_hour_in_boarding_time.expandedBoarding.value
                            )
                            row_dict[commune_str][stop_str][transport_modes_str][
                                auth_route_str
                            ][half_hour][0] = sum_trip_number
                ...
                for transport_modes in stop.transportModes:
                    ...
                    for auth_route in transport_modes.authRouteCode:
                        ...
                        for (
                            ...
                            sum_trip_number = (
                                half_hour_in_alighting_time.expandedAlighting.value
                            )
                            row_dict[commune_str][stop_str][transport_modes_str][
                                auth_route_str
                            ][half_hour][1] = sum_trip_number
...
class PostProductTripBoardingAndAlightingWithoutServiceCSVHelper(
...
class PostProductStageTransactionsByOperatorCSVHelper(CSVHelper):
    ...
    def get_file_description(self):
        description = (
            "Cada línea representa un conjunto de transacciones en una parada por día."
        )
        return "\t\t- {0}: {1}\r\n".format(self.get_data_file_name(), description)
    ...
    def get_column_dict(self):
        return [
            ...
            {
                "es_name": "timePeriodInBoardingTime",
                "csv_name": "Periodo_transantiago_subida",
                "definition": "Período transantiago en que inició el viaje",
            },
            {
            ...
            {
                "es_name": "expandedBoarding",
                "csv_name": "Número_de_transacciones",
    ...
    def row_parser(self, row):
        formatted_row = []
        op_program_date = ESShapeHelper().get_most_recent_operation_program_date(
            self.start_date
        )
                        ...
                        for bus_station in operator.busStation:
                            ...
                            formatted_row.append(
                                [
                                    ...
                                    operator_str,
                                    bus_station_str,
                                    round(bus_station.expandedBoarding.value, 2),
                                ]
                            )
        return formatted_row
